...
def level_DictComp(self):
    ...
    for i in self.node.generators:
        ...
        if numIfs > 0:
            ...
            self.clase = (
                "Dictionary Comprehension with " + str(numIfs) + " If statements"
            )
        else:
    ...
    if "ast.IfExp" in str(self.node.value):
        ...
        self.clase = (
            "Dictionary Comprehension with " + str(ifExp) + " if expression (If-Else)"
        )
...
def PrivateClass(self):
    ...
    for funct in self.node.body:
        ...
        if (funct.name.startswith("__")) and (not funct.name.endswith("__")):
            ...
            self.clase += " Private Methods " + str(funct.name) + " of the class"
        ...
        for i in ast.walk(funct):
            if "ast.Attribute" in str(i):
                if (i.attr.startswith("__")) and (not i.attr.endswith("__")):
                    self.level = dictLevel["Class"][5]["private"]
                    self.clase += " Private Attributes " + str(i.attr) + " of the class"
    ...
    # Class header
        ...
        for i in self.node.keywords:
            if i.arg == "metaclass":
                ...
                self.clase += (
                    " Metaclass created in the class header --> "
                    + "'metaclass = '"
                    + i.value.id
                )
    # As an attribute, 2.X
