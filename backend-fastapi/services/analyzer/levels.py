        ...
        for i in range(len(self.node.values)):
            try:
                if "ast.List" in str(self.node.values[i].values):
                    ...
                    self.clase = f"{num_list} List in {num_dict} Dictionary of Dictionary"
...
def level_constructor(self):
    ...
    if init_count > 0:
        self.clase += (
            f" using the constructor method → __init__ (x{init_count})"
            if init_count > 1
            else " using the constructor method → __init__"
        )
    ...
    if pos == "function":
        ...
        for keyword in self.node.keywords:
            if keyword.arg == "metaclass":
                ...
                self.clase += f" Metaclass created in the class header → 'metaclass = {keyword.value.id}'"
    elif pos == "attrib":
