...
USER_NAME = ""
API_KEY = ""
SETTINGS: Dict[str, Any] = {}  # TODO: Check, may be no longer necessary
...
def is_python_language(protocol: str, type_git: str) -> bool:
    """
...
def fetch_user(user: str) -> Dict[str, Any]:
    """
...
def fetch_user_repos(user: str) -> list[Dict[str, Any]]:
    """
...
def analyse_project(rootPath: str) -> None:
    """
    ...
    for root, dirs, files in os.walk(rootPath):
        dirs[:] = [
            d
            for d in dirs
            if not any(os.path.join(root, d).endswith(ignored_folder) for ignored_folder in ignored_folders)
        ]
        for file in files:
            if file.endswith(".py"):
                file_count += 1
...
def analyse_directory(path: str, total_length: Optional[int], current_file: Optional[list[int]]) -> None:
    """
...
# Check this, may be no longer necessary
def load_settings() -> None:
    ...
    SETTINGS["ignoreFolders"] = [
        folder.rstrip("/") for folder in SETTINGS.get("ignoreFolders", [])
    ]  # Remove / if included
...
def get_repo_data() -> Dict[str, Any]:
    ...
    return {"data": repo_data, "commits": repo_commits, "contributors": repo_contributors}


def calculate_hours_spent(
    commit_dates: list[float], max_commit_diff_seconds: int = 120 * 60, first_commit_addition_seconds: int = 120 * 60
) -> float:
    """
...
def run_directory(dir: str) -> None:
    """
    ...
    if git_url != "":
        while True:
            repo_input = (
                input(
                    "A valid Git configuration has been detected. Would you like to analyse the origin repository? (Y/n) "
                )
                .strip()
                .lower()
            )
...
def get_git_repo_url(dir: str) -> str:
    ...
    url: Optional[str] = config['remote "origin"'].get("url")
...
def get_repo_commits() -> list[Dict[str, Any]]:
    """
...
def fetch_commit_details(commit_url: str, headers: Dict[str, Any]) -> Dict[str, Any]:
...
def get_repo_contributors() -> list[Dict[str, Any]]:
    """
...
def save_data(data: Dict[str, Any]) -> str:
    ...
    This function loads existing data from `data.json`, updates it with the new repository information, and saves it to a new file in the `results/` directory. The new file will be named after the repository.

    Args:
        data (dict): The repository data to be saved, typically returned from `get_repo_data()`.

    Raises:
        SystemExit: If the `data.json` file cannot be found.
    """
    global REPO_NAME
